  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full border-b border-white/10 bg-black/40 backdrop-blur-2xl">
      <div className="mx-auto grid max-w-[1680px] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 px-3 py-2 sm:gap-4 sm:px-5 sm:py-3 md:grid-cols-[140px_minmax(0,1fr)_auto] md:gap-6 lg:grid-cols-[170px_minmax(0,1fr)_auto] lg:px-10">
        <button onClick={() => goToScene('opening')} aria-label="Go to home" className="flex h-10 w-auto sm:h-12 md:h-14 shrink-0 items-center overflow-visible">
          <Image src="/images/Logo.png" alt="Dubai Mall" width={260} height={100} priority sizes="(max-width: 640px) 100px, (max-width: 768px) 120px, 170px" className="h-10 sm:h-12 md:h-14 w-auto origin-left scale-[1.0] sm:scale-[1.1] md:scale-[1.2] object-contain transition-transform" />
        </button>
          ...
          {SCENES.filter((s) => s.id !== 'opening').map((scene, index) => {
            ...
            return (
              <button
                ...
                className={cn('group flex items-center gap-1 rounded-full px-2 sm:px-2.5 py-2 text-[9px] sm:text-[10px] uppercase tracking-[0.2em] sm:tracking-[0.22em] text-white/70 transition hover:text-white', active && 'text-white')}
              >
        ...
        <div className="flex items-center justify-end gap-2 self-center justify-self-end">
          ...
          <CTAButton variant="gold" className="h-8 sm:h-9 md:h-10 shrink-0 whitespace-nowrap px-3 sm:px-4 text-[8px] sm:text-[9px] md:text-[10px] leading-none" onClick={() => goToScene('contact')}>
            Contact
          </CTAButton>
        </div>
      </div>
